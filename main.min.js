...
(function () {
  ...
  if ('requestIdleCallback' in window) {
    requestIdleCallback(() => setTimeout(tick, 1500));
  } else {
...
(function () {
  ...
  let pts = [];
  let raf;
  const N = window.innerWidth < 600 ? 12 : window.innerWidth < 1200 ? 20 : 30;

  const resize = () => {
    W = canvas.width = window.innerWidth;
    H = canvas.height = window.innerHeight;
  };
  const mkPt = () => ({ x: Math.random() * W, y: Math.random() * H, vx: (Math.random() - .5) * .2, vy: (Math.random() - .5) * .2, l: Math.random() });
  ...
  const draw = () => {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      ctx.fillStyle = `rgba(${c}${Math.sin(p.l * Math.PI) * .4})`;
      ctx.fill();
    }
    ...
    for (let i = 0; i < pts.length; i += 2) {
      for (let j = i + 1; j < pts.length; j += 2) {
        ...
        if (d < 100) {
          ...
          ctx.strokeStyle = `rgba(${c}${(1 - d / 100) * .05})`;
          ctx.lineWidth = .4;
  resize();
